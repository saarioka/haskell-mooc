...
oddFactorial x
    |x < 0 = 0
    |x == 0 || x == 1 = 1
    |x `mod` 2 == 0 = oddFactorial (x-1)
    |x `mod` 2 == 1 = oddFactorial (x-2) * x
...
myGcd 0 x = x
myGcd x 0 = x
myGcd x y = myGcd ((max x y) - (min x y)) (min x y)
...
countdown' 0 s = s
countdown' n s = s ++ countdown' (n-1) ((show n) ++ "... ")
...
smallestDivisor' x n = if x `mod` n == 0 then n else smallestDivisor' x (n+1)
