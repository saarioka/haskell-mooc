...
sums :: Int -> [Int]
sums i = [sumsHelper i | i <- [1..i]]

sumsHelper :: Int -> Int
sumsHelper i
    | i == 0 = 0
    | otherwise = i + sumsHelper (i-1) 
...
indexDefault (x:xs) y def
    | y < 0 = def
    | y == 0 = x
    | otherwise = indexDefault xs (y-1) def
...
mymaximum bigger initial (x:xs) = 
    if x `bigger` initial then mymaximum bigger x xs else mymaximum bigger initial xs
