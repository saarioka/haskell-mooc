...
distinct xs = helper xs == xs 
  where
    helper [] = []
    helper (x : xs) = x : filter (/= x) (helper xs)
...
longest [x] = x
longest (x:xs)
  | length x > length longestTail = x
  | length x == length longestTail && head x <= head longestTail = x
  | otherwise = longestTail
  where longestTail = longest xs
...
incrementKey _ [] = []
incrementKey key ((k, v) : p)
  | key == k = (k, v + 1) : incrementKey key p
  | otherwise = (k, v) : incrementKey key p
...
freqs [] = Map.empty
freqs xs = foldr (Map.alter inc) Map.empty xs
...
transfer from to amount bank = 
  if Map.member from bank
    && Map.member to bank
    && amount > 0
    && fromJust (Map.lookup from bank) >= amount
     then Map.adjust (\old -> old + amount) to $ Map.adjust (\old -> old - amount) from bank
  else bank
...
maxIndex = fst . foldl1 (\(i, val) (maxI, maxVal) -> if val > maxVal then (i, val) else (maxI, maxVal)) . assocs 
