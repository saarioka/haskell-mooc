...
firstHalf s = reverse $ drop (length(s) `div` 2) (reverse s)
...
powers' k max i = if k^i <= max then k^i : powers' k max (i+1) else []
...
joinToLength i s = [s1 ++ s2 | s1 <- s, s2 <- s, length(s1 ++ s2) == i]
...
--sumRights x = sum (map (either (const 0) id) x)
...
interpreter c = m c 0 0
  where
    m ("up" : c) x y = m c x (y + 1)
    m ("down" : c) x y = m c x (y -1)
    m ("left" : c) x y = m c (x -1) y
    m ("right" : c) x y = m c (x + 1) y
    m ("printX" : c) x y = show x : m c x y
    m ("printY" : c) x y = show y : m c x y
    m [] x y = []
